...
socdemResults <- function(x = nhhmem, ord, weight, data, breakdown = seq(0, 1, by = 0.1), imp) {

  # List of results based on household groups
  res <- list("person"=c(rep(NA,(length(breakdown)-1))),"households"=c(rep(NA,(length(breakdown)-1))))

  for (i in 2:length(breakdown)) {
    result <- percentiles(x = {{x}}, ord = {{ord}}, weight = {{weight}}, xUp = breakdown[i - 1], xLow = breakdown[i], data = data, imp = {{imp}})
